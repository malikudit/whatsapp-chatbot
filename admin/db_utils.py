...
global DEFAULT_PATH
DEFAULT_PATH = os.path.join(os.path.dirname(__file__), 'shopDb.db')
...
def insert_in_table(conn, item):
    sql = """ INSERT INTO shopDb(phone_no, shop_name, shopkeeper_name, shop_pincode, shop_address)
    VALUES(?,?,?,?,?)
    """
    cur = conn.cursor()
...
def main():
    ...
    with conn:
        ...
        while resp == "yes":
            decision = input("Type delete, add, or show:  ")
            decision = decision.strip().lower()

            if decision == "add":
                phone = input("Enter the shopkeeper's phone number: ")
                shop_name = input("Enter the shop's name: ")
                shopkeeper = input("Enter the shopkeeper's name, if available: ")
                pin = input("Enter the pincode of the shop: ")
                address = input("Enter the shop's address, if available: ")
                item = (phone, shop_name, shopkeeper, pin, address)
